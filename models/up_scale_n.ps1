...
function InvoqueRealesrgan {
    ...
    $argument = "-i `"$in_frames`" -o `"$out_frames`" -n realesr-animevideov3 -s 2 -f jpg" 
...
function ImageMagic {
    ...
    $argument = "convert $out_frame -modulate $brillo  -level $contraste  $filtered_frame"
...
function Main {
    ...
    foreach ($video in $lista_videos) {
        ...
        InvoqueRealesrgan -realesrgan $realesrgan -in_frames $in_frames -out_frames $out_frames 
